namespace Raft {
    ...
    public class ImageWrapper : IDisposable {
        private ImageWrapper(Image image, DeviceMemory memory, ImageView view, Format format) {
            Image = image;
            Memory = memory;
            View = view;
            Format = format;
        ...
        public static implicit operator Image(ImageWrapper value) => value.Image;

        public static ImageWrapper DepthStencil(Context device, int width, int height) {
            Format[] validFormats = {
            ...
            return new ImageWrapper(image, memory, view, format);
        }

        internal static ImageWrapper Texture2D(Context ctx, TextureData tex2D) {
            ...
            return new ImageWrapper(image, memory, view, tex2D.Format);
        }
    }
}